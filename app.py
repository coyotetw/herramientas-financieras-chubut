...
# ── NAV ──
TABS = ["Actividades", "Programas", "Créditos", "Calendario", "Observatorio", "Consulta CUIT"]
ICONS = ["", "", "", "", "", ""]
...
# ════════════════════════════════════
    ...
    from modulo_cuit import procesar_lote, exportar_excel, MAX_CUITS, SEMAFORO_COLORES, validar_cuit, limpiar_cuit
    ...
    <div style="padding:48px 48px 0;">
      ...
      <p style="font-size:13px;color:#8BA5C0;margin-bottom:28px;">
        Consulta simultánea de constancia de inscripción en ARCA (ex AFIP) y situación 
        en la Central de Deudores del BCRA. Hasta 5 CUITs por lote.
      </p>
    </div>
    ''', unsafe_allow_html=True)
    ...
    with col_inp:
        cuits_texto = st.text_area(
            "Ingresá los CUITs (uno por línea, con o sin guiones):",
            height=140,
            placeholder="27123456789\n20987654321\n30-71234567-8",
            help=f"Máximo {MAX_CUITS} CUITs por consulta"
        )
    with col_btn:
        st.markdown("<div style='height:28px;'></div>", unsafe_allow_html=True)
        consultar = st.button("Consultar", use_container_width=True)
        st.markdown(
            '<div style="font-size:11px;color:#4A6080;margin-top:8px;font-family:Space Mono,monospace;">'
            f'MAX {MAX_CUITS} CUITs</div>',
            unsafe_allow_html=True
        )

    if consultar and cuits_texto.strip():
        cuits_raw = [l.strip() for l in cuits_texto.strip().splitlines() if l.strip()]

        if len(cuits_raw) > MAX_CUITS:
            st.warning(f"Ingresaste {len(cuits_raw)} CUITs. Se procesarán solo los primeros {MAX_CUITS}.")
            cuits_raw = cuits_raw[:MAX_CUITS]

        # Validación previa
        invalidos = []
        for c in cuits_raw:
            cl = limpiar_cuit(c)
            ok, motivo = validar_cuit(cl)
            if not ok:
                invalidos.append(f"{c} → {motivo}")
        ...
        if invalidos:
            st.markdown(
                ...
                'letter-spacing:0.08em;margin-bottom:8px;">CUITS CON ERROR DE VALIDACIÓN</div>' +
                "".join([f'<div style="font-size:12px;color:#F87171;margin-bottom:4px;">⚠ {e}</div>' for e in invalidos]) +
                '</div>',
                unsafe_allow_html=True
            )

        cuits_validos = [limpiar_cuit(c) for c in cuits_raw if validar_cuit(limpiar_cuit(c))[0]]

        if cuits_validos:
            prog_bar = st.progress(0)
            prog_txt = st.empty()

            def actualizar_progreso(i, total, cuit):
                prog_bar.progress((i) / total)
                prog_txt.markdown(
                    f'<div style="font-family:Space Mono,monospace;font-size:11px;color:#5BB8D4;">'
                    f'Consultando {cuit} ({i+1}/{total})...</div>',
                    unsafe_allow_html=True
                )

            resultados = procesar_lote(cuits_validos, progress_callback=actualizar_progreso)
            prog_bar.progress(1.0)
            prog_txt.markdown(
                f'<div style="font-family:Space Mono,monospace;font-size:11px;color:#34D399;">'
                f'Consulta completada — {len(resultados)} CUITs procesados</div>',
                unsafe_allow_html=True
            )
            ...
            for res in resultados:
                ...
                if not res["valido"]:
                    st.markdown(
                        f'<div class="cred-card" style="border-left-color:#EF4444;">' +
                        f'<div class="cred-name">{cuit}</div>' +
                        f'<div class="cred-dest">CUIT inválido: {res.get("error_validacion","")}</div>' +
                        '</div>', unsafe_allow_html=True
                    )
                    continue

                with st.expander(
                    f"{arca.get('nombre', cuit) or cuit}  —  {cuit}",
                    expanded=True
                ):
                    # Semáforo + tiempos
                    st.markdown(
                    ...
                    with col_b:
                        ...
                        if bcra.get("ok"):
                            if bcra.get("sin_deuda"):
                                ...
                                sit_color = SEMAFORO_COLORES.get(
                                    ["verde","verde","amarillo","naranja","rojo","rojo"][min(bcra.get("situacion",1),5)],
                                    "#6B7280"
                                )
                                filas_bcra = [
            ...
            excel_buf = exportar_excel(resultados)
            st.download_button(
                label="Exportar resultados a Excel",
                data=excel_buf,
                file_name=f"consulta_cuit_{pd.Timestamp.now().strftime('%Y%m%d_%H%M')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )

            # ── Resumen de tiempos (transparencia de consumo) ──
            t_total_lote = sum(r.get("tiempo_total",0) for r in resultados)
            st.markdown(
                f'<div style="margin-top:16px;font-family:Space Mono,monospace;font-size:10px;color:#2A3D55;'
                f'text-align:right;">Tiempo total de consulta: {t_total_lote:.1f}s · APIs: ARCA + BCRA (gratuitas)</div>',
                unsafe_allow_html=True
            )
