...
def init_db():
    ...
    # ── NOVEDADES_CARRUSEL (banners destacados en Actividades) ──
    cur.execute("""
        CREATE TABLE IF NOT EXISTS novedades_carrusel (
            id SERIAL PRIMARY KEY,
            foto TEXT,
            texto TEXT NOT NULL,
            link TEXT,
    ...
    # ── EVENTOS (listado de próximos / realizados en Actividades) ──
    cur.execute("""
...
# ─────────────────────────────────────
# NOVEDADES_CARRUSEL (Leer: libre · Crear/Modificar/Borrar: requieren PIN, validado en la capa de app)
# ─────────────────────────────────────
def listar_novedades_carrusel(solo_activas=True):
    conn = get_conn()
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    if solo_activas:
        cur.execute("SELECT * FROM novedades_carrusel WHERE activa = TRUE ORDER BY orden, id;")
    else:
        cur.execute("SELECT * FROM novedades_carrusel ORDER BY orden, id;")
    rows = [dict(r) for r in cur.fetchall()]
    cur.close()
    conn.close()
    return rows
...
def crear_novedad_carrusel(foto, texto, link, fecha_evento, orden=0, activa=True):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO novedades_carrusel (foto, texto, link, fecha_evento, orden, activa)
        VALUES (%s,%s,%s,%s,%s,%s) RETURNING id;
    """, (foto, texto, link, fecha_evento, orden, activa))
    new_id = cur.fetchone()[0]
    conn.commit()
    cur.close()
    conn.close()
...
def actualizar_novedad_carrusel(novedad_id, foto, texto, link, fecha_evento, orden, activa):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        UPDATE novedades_carrusel SET
            foto=%s, texto=%s, link=%s, fecha_evento=%s, orden=%s, activa=%s,
            actualizado=NOW()
        WHERE id=%s;
    """, (foto, texto, link, fecha_evento, orden, activa, novedad_id))
    conn.commit()
    cur.close()
    conn.close()


def borrar_novedad_carrusel(novedad_id):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("DELETE FROM novedades_carrusel WHERE id=%s;", (novedad_id,))
    conn.commit()
    cur.close()
    conn.close()
...
# ─────────────────────────────────────
# EVENTOS (Leer: libre · Crear/Modificar/Borrar: requieren PIN, validado en la capa de app)
# ─────────────────────────────────────
def listar_eventos():
    conn = get_conn()
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("SELECT * FROM eventos ORDER BY fecha_evento;")
    rows = [dict(r) for r in cur.fetchall()]
    cur.close()
    conn.close()
    return rows
...
def crear_evento(foto, fecha_evento, texto, link):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO eventos (foto, fecha_evento, texto, link)
        VALUES (%s,%s,%s,%s) RETURNING id;
    """, (foto, fecha_evento, texto, link))
    new_id = cur.fetchone()[0]
    conn.commit()
    cur.close()
    conn.close()
...
def actualizar_evento(evento_id, foto, fecha_evento, texto, link):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        UPDATE eventos SET
            foto=%s, fecha_evento=%s, texto=%s, link=%s, actualizado=NOW()
        WHERE id=%s;
    """, (foto, fecha_evento, texto, link, evento_id))
    conn.commit()
    cur.close()
    conn.close()
